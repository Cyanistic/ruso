...
pub fn SettingsTab(cx: Scope) -> Element{
    ...
    let used_space = use_state(cx, || match read_space(){
        Ok(k) => k,
        Err(e) => {
    ...
    let clean_text = use_memo(cx, clean_confirmation.get(), |confirm|{
        match confirm{
            true => "Are you sure?",
            false => "Clean maps"
        }
    ...
    cx.render(rsx!{
        ...
        div {
            ...
            br {}
            button {
                    title: "Calculate used space: Calculates the amount of space used by the .osu and audio files generated by ruso",
                    onclick: move |_| {
                        match calculate_space(){
            ...
            p{ style: "display: inline;", " {used_space_pretty}" }
            br {}
            div{
