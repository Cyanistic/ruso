use std::{process::{exit, Command, Child}, path::PathBuf, io::{stdout, IsTerminal, stderr, stdin, Write}, time::Duration};
...
pub async fn run() -> Result<()>{
    ...
    // Create my own arg parser because clap is bloated
    ...
    // Invalidate flags if they have arguments
    {   let mut count: usize = 0;
        for (ind, val) in args.clone().iter().enumerate(){ 
            if FLAGS.contains(val){
                match args.get(ind+count+1){
                    Some(x) if AVAILABLE_COMMANDS.contains(x) => {
                        args.insert(ind+count+1, "");
                        count += 1;
                    },
                    None => args.insert(ind+count+1, ""),
                    Some(x) => return Err(anyhow!("{} is a flag and does not take any arguments.\nUse '{}' instead of '{} {}'.", args[ind+count], args[ind+count], args[ind+count], x)),
                }
            }
        }
    }
    ...
    for i in (0..args.len()).step_by(2){
        if !AVAILABLE_COMMANDS.contains(&args[i]){
            return Err(anyhow!("Invalid command: {}\n Run 'ruso --help' for help information.", args[i]));
        } else if !FLAGS.contains(&args[i]){
            match args.get(i+1){
                Some(x) if AVAILABLE_COMMANDS.contains(x) => return Err(anyhow!("Option '{}' requires a value but none were provided.", args[i])),
                None => return Err(anyhow!("Option '{}' requires a value but none were provided.", args[i])),
                Some(_) => ()
            }
        }
    }
    ...
    let mut gosu_process: Option<Child> = None;
    for ind in (0..args.len()).step_by(2){
        match args[ind]{
            ...
            "-b"| "--bpm" => bpm = Some(match args[ind+1].parse::<usize>(){
                Ok(k) => k,
                Err(e) => return Err(anyhow!("Could not parse bpm: {}\nNote that bpm values must be positive integers.", e))
            }),
            "-c"| "--circle-size" => {
            ...
            "-C"| "--clean" => {
                println!("Are you sure you want to delete ALL maps and audio files generated by ruso? [y/N]");
